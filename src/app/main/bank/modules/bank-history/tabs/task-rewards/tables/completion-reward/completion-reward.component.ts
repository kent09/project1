...
import { BANK_API, SuperCoinHttpRequest } from 'app/main/bank/modules/bank-history/tabs/task-rewards/tables/completion-reward/completion-reward.service';
import { CompletionModalComponent } from './completion-modal/completion-modal.component';

@Component({
  selector: 'app-bank-completion-reward',
  templateUrl: './completion-reward.component.html',
  styleUrls: ['./completion-reward.component.scss'],
  encapsulation: ViewEncapsulation.None,
  animations: fuseAnimations
})
export class BankCompletionRewardTableComponent implements OnInit {

  displayedColumns = ['category', 'avatar', 'creator', 'task_title', 'completed_date', 'reward', 'status', 'option'];
  ...
  openDialog(detial): void {
    const dialogRef = this._dialog.open(CompletionModalComponent, {
      data: detial
    });
  }
}
